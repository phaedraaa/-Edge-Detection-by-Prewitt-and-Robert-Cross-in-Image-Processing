...
void apply_roberts()
{
    ...
    for (i = 0; i < N - 1; i++)
    {
        for (j = 0; j < M - 1; j += 2) // Unrolling by a factor of 2
        {
            // First pixel
            gx = 0;
            gy = 0;

            gx += current_y[i][j] * Gx_Roberts[0][0];
            gx += current_y[i][j + 1] * Gx_Roberts[0][1];
            gx += current_y[i + 1][j] * Gx_Roberts[1][0];
            gx += current_y[i + 1][j + 1] * Gx_Roberts[1][1];

            gy += current_y[i][j] * Gy_Roberts[0][0];
            gy += current_y[i][j + 1] * Gy_Roberts[0][1];
            gy += current_y[i + 1][j] * Gy_Roberts[1][0];
            gy += current_y[i + 1][j + 1] * Gy_Roberts[1][1];
            ...
            if (j + 1 < M - 1) // Ensure within bounds
            {
                ...
                gx += current_y[i][j + 1] * Gx_Roberts[0][0];
                gx += current_y[i][j + 2] * Gx_Roberts[0][1];
                gx += current_y[i + 1][j + 1] * Gx_Roberts[1][0];
                gx += current_y[i + 1][j + 2] * Gx_Roberts[1][1];

                gy += current_y[i][j + 1] * Gy_Roberts[0][0];
                gy += current_y[i][j + 2] * Gy_Roberts[0][1];
                gy += current_y[i + 1][j + 1] * Gy_Roberts[1][0];
                gy += current_y[i + 1][j + 2] * Gy_Roberts[1][1];

                edge_roberts[i][j + 1] = (int)sqrt(gx * gx + gy * gy);
                if (edge_roberts[i][j + 1] > 255)
                    edge_roberts[i][j + 1] = 255;
            }
        }
    }
}

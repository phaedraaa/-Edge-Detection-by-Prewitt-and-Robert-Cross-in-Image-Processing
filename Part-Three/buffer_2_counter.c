...
int buffer_row_1_access;
int buffer_row_2_access;
int buffer_block_2x3_access;
int buffer_block_2x2_access;
#pragma arm section
int i, j;
...
/* Apply Roberts Operator using buffers (Schema 6) */
void apply_roberts_schema_6()
{
    int x, y, gx, gy;
    ...
    for (j = 0; j < M - 1; j++)
    {
        for (i = 0; i < N - 1; i++)
        {
            ...
            // Load 2x2 block into buffer_block_2x2
            for (y = 0; y < 2; y++)
            {
                for (x = 0; x < 2; x++)
                {
                    buffer_block_2x2[y][x] = current_y[i + x][j + y];
                    buffer_block_2x2_access++;
                }
            }
            ...
            for (y = 0; y < 2; y++)
            {
                for (x = 0; x < 2; x++)
                {
                    gx += buffer_block_2x2[y][x] * Gx_Roberts[x][y];
                    gy += buffer_block_2x2[y][x] * Gy_Roberts[x][y];
                }
            }
...
int main()
{
    ...
    /* Apply Prewitt Operator using Schema 6 */
    apply_prewitt_schema_6();

    /* Apply Roberts Operator using Schema 6 */
    apply_roberts_schema_6();

    /* Write results */
    write_prewitt();
    write_roberts();

    /* Print memory access counts */
    printf("Memory access counts:\n");
    printf("buffer_row_1_access: %d\n", buffer_row_1_access);
    printf("buffer_row_2_access: %d\n", buffer_row_2_access);
    printf("buffer_block_2x3_access: %d\n", buffer_block_2x3_access);
    printf("buffer_block_2x2_access: %d\n", buffer_block_2x2_access);
